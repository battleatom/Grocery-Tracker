...
const FILE='public/data/prices.json',HISTORY='public/data/history.json';
...
for(const src of sources){try{const res=await fetch(src.url,{headers:{'user-agent':'Mozilla/5.0 GroceryTracker/1.0'}});if(!res.ok)continue;const html=(await res.text()).replace(/&cent;/g,'¢').replace(/&amp;/g,'&');for(const [id,re,price,unit_price,unit] of rules[src.store]){if(!re.test(html))continue;const item=data.items.find(x=>x.id===id);if(!item)continue;const old=item.offers[src.store];item.offers[src.store]={...(old||{}),price,unit_price,unit,source:src.name,observed:now.slice(0,10),automated:true};history.push({at:now,item:id,store:data.meta.stores[src.store],price,unit_price,unit});changed++}}catch(e){console.error(src.name,e.message)}}
data.meta.automation={last_run:now,matched:changed,schedule:'every 6 hours',policy:'retain last verified value when a source cannot be parsed; last_run is a check time, not a price freshness claim'};if(changed>0)data.meta.updated_at=now;
